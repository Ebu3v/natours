...
const displayMap = locations => {
  ...
  for (let i = 0; i < locations.length; i++) {
    const popup = new maptilersdk.Popup({
      offset: 25
    }).setHTML("<p>Day ".concat(locations[i].day, ": ").concat(locations[i].description, " </P>"));
    new maptilersdk.Marker().setLngLat(locations[i].coordinates).setPopup(popup).addTo(map);
  }
};
...
function resolveConfig(config) {
  ...
  newConfig.url = (0, _buildURL.default)((0, _buildFullPath.default)(baseURL, url, allowAbsoluteUrls), own('params'), own('paramsSerializer'));
...
const VERSION = exports.VERSION = "1.17.0";
...
// ntype is either 'password' or 'data'
...
// export the function and call it in index.js
...
const stripe = Stripe('pk_test_51TjHWAAoREbEbffeoLcQy5uUAAtsg3gs2rn5vjf2U9WWFDXnmnlZGE6EQEWigyhp4oMmuc5AUIaeGzYZp1c6v2Ce00EbICXdKK');
